...
| `energy` | Energia della sessione in **Wh**, col **segno come flag** (vedi sotto) |
...
### Il campo `energy`: attenzione all'unita' e al segno

Non e' in kWh, e il segno non fa parte del valore. Dal JavaScript della centralina:

```javascript
if (data.energy > 0) { stimato = 0; }               // misurata
else { stimato = 1; data.energy = -data.energy; }   // STIMATA
document.getElementById("energy").innerHTML = data.energy + "&nbsp;Wh";
```

Quindi: **unita' Wh**, e il segno distingue una misura da contatore reale
(positivo) da una stima della centralina (negativo, col modulo come valore).

Verificato durante una ricarica reale: `energy = -372` con `pcar = 7360` e
`time = 182` -> 7360 W x 182 s = 372 Wh esatti. Questo esemplare riporta
**sempre** valori stimati: non ha un contatore reale a bordo.

Per usarlo: `abs(energy) / 1000` per i kWh, e il segno solo come flag.
Trattarlo come kWh con segno produce numeri negativi e sbagliati di 1000 volte.

### Nota sugli algoritmi

`alg` va tenuto a `2` (Man) perche' il regolatore possa imporre la potenza.
Se qualcuno lo cambia dal pannello della centralina, `btn=P<watt>` viene
accettato ma ignorato, senza alcun errore.
